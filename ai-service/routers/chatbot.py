"""Chatbot router — Gemini-powered pregnancy assistant with local fallback."""
...
async def chat(request: ChatRequest):
    ...
    try:
        ...
        if request.pregnancy_week:
            prompt_parts.append(f"The user is currently in week {request.pregnancy_week} of pregnancy.")
        if request.context:
        ...
        logger.warning("Gemini request failed, using local fallback: %s", e)
        # Fall back to local if Gemini fails
        reply = _get_local_response(request.message)
        return ChatResponse(
            message=reply,
            suggested_actions=_extract_actions(request.message),
