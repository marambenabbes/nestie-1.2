...
@Component({
  selector: 'app-symptom-calendar',
  standalone: true,
  imports: [CommonModule, MatIconModule],
  template: `
    <div class="rounded-3xl bg-gradient-to-br from-rose-50 via-pink-50 to-fuchsia-50 border border-pink-100/60 shadow-lg p-5">
      ...
      <div class="grid grid-cols-7 gap-1.5">
        ...
        >
          ...
          <div class="absolute bottom-1 left-1/2 -translate-x-1/2 flex flex-col items-center gap-0.5" *ngIf="cell.count > 0">
            <div class="w-5 h-5 rounded-full flex items-center justify-center shadow-sm" [ngClass]="getSeverityDot(cell.maxSeverityWeight)">
...
export class SymptomCalendarComponent implements OnChanges {
  @Input() entries: SymptomEntry[] = [];
  ...
  private buildCell(date: Date, inCurrentMonth: boolean): CalendarCell {
    ...
    const dayEntries = this.entries.filter(e => {
      const entryDate = new Date(e.occurredAt).toISOString().split('T')[0];
    ...
    return {
      ...
      maxSeverityWeight: dayEntries.reduce((max, e) => Math.max(max, this.severityWeight(e.severity)), 0)
    };
  }
  ...
  getSeverityDot(weight: number): string {
